...
k=4*10^3;
L=1;
I0=(1/12)*m*L^2+m*((L/2)-(L/3))^2;

#Primero debemos ingresar la matriz M, C y K de manera manual
N=input('Ingrese el numero de grados de libertad de su sistema: ');
M=[I0 0 0; 0 m 0; 0 0 2*m]; #Modificar manualmente segun los grados de libertad que tenga
K=[(2*k*(L/3)^2+2*k*(2*L/3)^2) ((k*L/3)) (-(2*k*L/3)) ; (k*L/3) k 0; -(2*k*L/3) 0 k]#Idem
#Lo mismo para los vectores de condiciones iniciales X(0)=X0 y X'(0)=dX0
X0=[0.523599;0;0]; #30 grados son 0.523599 rad
dX0=[0;0;0];
...
disp(wn);
tf=5; #Ingresamos manualmente el tiempo final hasta donde queremos calcular el despl.
...
if (pregamort=='No')
  ...
  qinicial=Xn'*M*X0;
  disp('Las condiciones iniciales son: ')
  disp(qinicial)
  dqinicial=Xn'*M*dX0;
  disp(dqinicial)
...
x=Xn*q;
disp('El valor max de tita entre 0.5 y 1.5 es :')
disp(max(x(3,:)));
#disp('El valor max de x1 entre 2 y 3 es: ')
#disp('El valor max de x2 entre 2 y 3 es: ')
