function multiplesGrados_copia
  clc
  clear all
  close all
  ...
  %-----------------------------------CARGAS--------------------------------------
    ...
    tf=3; #Tiempo final (introducir el valor al que se quiere obtener)
    ...
    contador=1;
    for t=tiempo
      F(:,contador)=[10*sin(frecWf(1)*t),0,0]; #Esta solo sirve para printear la carga
      phi=atan((2*Ramort(1)*beta(1))/(1-beta(1)^2));
      Fauxresp(:,contador)=[10*sin(frecWf(1)*t-phi),0]; #Esta es la que pasamos porque tiene el desfase
      contador++;
    endfor
