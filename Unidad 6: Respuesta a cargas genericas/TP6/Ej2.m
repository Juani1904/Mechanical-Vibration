function cargasGenericas
  ...
  dt=0.025;  #Ingrese el paso de muestreo de la fuerza
  tf=1.2;  #Ingrese hasta que tiempo vamos a medir desplazamiento (en segundos)
  tiempo=ti:dt:tf;
  Ftau=[0,430000,0]; #Vector de fuerza tau que ira en yc e ys
  Ftau=cat(2,Ftau,zeros(1,length(tiempo)-length(Ftau)));
  #La frecuencia natural puede ser dato o no.
  m=250; #Masa
  k=4000*10^4 ; #Ctte de ridigez
  wn=sqrt(k/m);
  #c=0.4; #Ctte de amortiguamiento c. En caso de no haber amortiguamiento dejar en 0
  Ramort=0.05; #Si c no es dato simplemente colocar el valor de Ramort(Zitta)
  %----------------------------------------------------%
